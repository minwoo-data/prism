...
**한 명의 리뷰어 시선으로는 놓쳤던 문제가 나중에 터져서 후회한 적 있나요?** prism은 5명의 리뷰어가 각자 다른 각도(충돌 탐지 / 개선 / Devil's advocate / 코드 리뷰 / 견고성)에서 병렬로 분석하고, 한 명만 지적한 finding은 별도 검증을 거쳐 false positive를 걸러냅니다.
...
`/prism src/services/auth.py`. 5명의 리뷰어가 병렬로 서로 다른 편향(충돌 탐지 / 개선 / Devil's advocate / 코드 리뷰 / 견고성)에서 파일을 봅니다. 2명 이상이 같은 지적을 하면 자동 CONFIRMED. 한 명만 본 지적은 별도 Verifier pass를 거쳐 false positive가 시간을 낭비하지 않게 합니다.
